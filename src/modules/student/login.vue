<template>
  <main class="flex">
    <div class="login">
      <header>
        <h1>
          <svg role="img" class="logo" height="50" viewBox="0 0 241 60">
            <title>Teach Starter Assess</title>
            <defs>
              <linearGradient id="lg1" x1="99.631%" x2="66.75%" y1="49.933%" y2="49.933%">
                <stop offset="0%" stop-color="rgb(220,220,220)" />
                <stop offset="100%" stop-color="rgb(255,255,255)" />
              </linearGradient>
              <linearGradient id="lg2" x1=".369%" x2="33.25%" y1="50.201%" y2="50.201%">
                <stop offset="0%" stop-color="rgb(220,220,220)" />
                <stop offset="100%" stop-color="rgb(255,255,255)" />
              </linearGradient>
            </defs>
      ...
      <form @submit.prevent="login" action="#" method="post" :class="{ error: toastMessage != '' }">
        <p>
          <label for="username">Username</label>
          <input type="text" id="username" v-model="username" autofocus required />
        </p>
        <p>
        ...
        <div class="bounce">
          <button type="submit" class="button button-primary">Student Log In</button>
        </div>
      </form>
      <p><small>Forgot your username or password? Ask your teacher</small></p>
    </div>
...
<script>
/*
 * Authenticates a student via email and password
 *
 * @parent: /
 * @requests:
 * @events: loginStudent, track
 * @props:
    ...
    async login() {
      if (this.username != '' && this.password != '') {
        try {
          ...
          // Only redirect links that start with /student
          const redirect =
            this.redirect && this.redirect.startsWith('/student') ? this.redirect : '/student';
