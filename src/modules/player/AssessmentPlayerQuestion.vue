  ...
  <div class="player-question">
    <header class="player-question-title">
      ...
      <h3>
        ...
        <text-to-speech :text="getQuestionText" />
      </h3>
    ...
    <p
      v-if="
        automarkQuestions.includes(question.type) &&
          hasResults &&
          question.settings &&
          question.settings.showAnswer &&
          isCorrect === null
      "
    >
 ...
 * @props: Number assessmentId, Number index, Object question, Number currentIndex, Number resultId, Array savedResult, Boolean storeResult
...
export default {
  ...
  props: {
    ...
    currentIndex: Number,
    resultId: Number,
    savedResult: [Array, String], // Text question result is a string, the rest are arrays
    storeResult: Boolean, // Store this result or not (default is false for teacher)
  },
  components: {
    QuestionMcq,
    QuestionText,
  ...
  methods: {
    ...
    async showAnswer() {
      // API query returns a simple true/false if the question is correct/incorrect
      this.isCorrect = null;
  ...
  computed: {
    ...
    getQuestionText() {
      // Contruct the script to read aloud when the voice button is clicked
      let text = '';

      if (this.question.type != 'statement') {
        text = 'Question ' + (this.index + 1) + ': ' + this.question.label + '. ';

        // If title is the same as the label then don't repeat it
        if (this.question.title != '' && this.question.title != this.question.label) {
