<template>
  <section>
    <h1>
      <svg role="img" height="50" viewBox="0 0 241 60">
        <title>Teach Starter Assess</title>
        <defs>
          <linearGradient id="logo-assess-a" x1="99.631%" x2="66.75%" y1="49.933%" y2="49.933%">
            <stop offset="0%" stop-color="rgb(105,163,64)" />
            <stop offset="100%" stop-color="rgb(120, 188, 58)" />
          </linearGradient>
          <linearGradient id="logo-assess-b" x1=".369%" x2="33.25%" y1="50.201%" y2="50.201%">
            <stop offset="0%" stop-color="rgb(105,163,64)" />
            <stop offset="100%" stop-color="rgb(120, 188, 58)" />
          </linearGradient>
        </defs>
    ...
    <div v-if="authorised" class="box">
      <span class="text-mute" v-if="isLoading">
        Sharing activity with you...
      </span>
      <template v-else-if="assessmentCreated">
        <h3>Success! "{{ assessmentTitle }}" was added to your activities.</h3>
        <router-link :to="'/assess/edit/' + assessmentSlug" class="button button-primary"
          >Check it out ({{ timeToRedirect }})</router-link
        >
      </template>
      <template v-else-if="errorMessage">
        <h3>Error sharing activity</h3>
        {{ errorMessage || 'Something went wrong' }}
        <router-link :to="'/assess/'" class="button button-primary margin-top">
    ...
    <div v-else class="flex flex-wrap">
      <div class="not-logged-in">
        <img src="/images/share-assessment-template.png" alt="" class="hidden-small" />

        <h3 v-if="assessmentTitle">{{ assessmentTitle }}</h3>
        <p>
          Someone shared an Assess activity <span>with you!</span><br class="hidden-small" />
          Login to add it to your activity list.
        </p>
      </div>
      <div>
        <div class="box">
          <login-form button="Log In" :reload="true" />
        </div>

        <br /><small class="text-center">
          Need an account?
          <a
            href="https://www.teachstarter.com/subscribe/?plan=free"
            title="Create a Teach Starter account"
            >Sign up here</a
          >
        </small>
      </div>
...
export default {
  ...
  created() {
    if (this.authorised && this.shareToken !== '') {
      this.createAssessment();
    } else if (!this.authorised) {
      // Redirect back to this page after login
      localStorage.setItem(STORAGE_KEY_REDIRECT, this.$route.fullPath);
    }
  },
  methods: {
    async createAssessment() {
      // Send the token to validate. Create a new slug for the activity
      const params = {
      ...
      try {
        const response = await this.$store.dispatch('createAssessmentCopy', params);

        this.assessmentCreated = true;
        this.isLoading = false;

        this.assessmentSlug = response.slug;
    ...
    countdown() {
      if (!this.assessmentSlug) return;

      this.timeToRedirect = this.timeToRedirect - 1;

      if (this.timeToRedirect <= 0) {
        // Redirect to edit page after X seconds
        window.location.replace('/assess/edit/' + this.assessmentSlug);
      } else {
