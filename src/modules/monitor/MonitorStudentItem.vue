    ...
    >
      ...
      <td data-title="Score">
        ...
        <router-link
          ...
          title="Open this result to grade it"
          >Grade</router-link
        >
      </td>
      <td data-title="Grade">
      ...
      <td data-title="Graded">
        <span v-if="result.marked" class="graded">
      ...
      <td data-title="Curriculum">
        <template v-if="result.curriculumTags">
          <span v-for="tag in result.curriculumTags" :key="tag.id" class="label">{{
            tag.name
          }}</span>
        </template>
      </td>
 ...
 * @parent: /monitor/view
 * @requests:
 * @events: select-result, view result (link), view assessment (link)
 * @props: Number assessmentId, String assessmentTitle, Boolean isSelected, Array results
 * @components:
 * @methods: getPercentage, linkTo, selectResult, updateNotes
 */
